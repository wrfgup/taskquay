...
test("SDK HTTP recovery: bounded Unicode capture, exact reconstruction, command/patch receipts, restart, old get and failed followup", { timeout: 60000 }, async (t) => {
  ...
  const call = async (name: string, args: Record<string, unknown>) => {
    const result = await client.callTool({ name, arguments: { workspaceId: "ws_abcdef", workRunId: run.id, ...args }, _meta: { "openai/session": "private-conversation-secret" } });
  ...
  assert(listed.find((x) => x.name === "workspace_context")!.inputSchema.properties!.lineOffset);
  const files = [{ path: "source0.txt", maxLines: 250 }, { path: "source1.txt", maxLines: 250 }];
  let next: any = { selectionIndex: 0, startLine: 1, lineOffset: 0 }, pages = 0;
  const rebuilt = ["", ""];
  while (next) {
    const selections = files.map((s, i) => ({ ...s, startLine: i === next.selectionIndex ? next.startLine : 1 }));
    const page = await call("workspace_context", { action: "capture", files: selections, selectionIndex: next.selectionIndex, lineOffset: next.lineOffset });
  ...
  const a = await call("workspace_context", { action: "capture", files: [{ path: "source1.txt", startLine: 1 }] });
  const b = await call("workspace_context", { action: "capture", files: [{ path: "source1.txt", startLine: 2 }] });
  ...
  const terminal = await call("exec_command", { cmd: `"${process.execPath}" -e "process.stdout.write('command-once')"`, yieldTimeMs: 2000, requestKey: "command-once" });
  assert.equal(terminal.exitCode, 0);
  const nonzero = await call("exec_command", { cmd: `"${process.execPath}" -e "process.exit(7)"`, yieldTimeMs: 2000 });
  assert.equal(nonzero.exitCode, 7);
  const patch = await call("apply_patch", { patch: "*** Begin Patch\n*** Add File: changed.txt\n+changed once\n*** End Patch", requestKey: "patch-once" });
  assert(patch.operationId);
  ...
  for (const [name, args] of [["exec_command", { requestKey: "command-once", cmd: "echo must-not-replay" }],
    ["apply_patch", { requestKey: "patch-once", patch: "*** Begin Patch\n*** Delete File: changed.txt\n*** End Patch" }]] as const) {
    const replay = await client.callTool({ name, arguments: { workspaceId: "ws_abcdef", workRunId: run.id, ...args } });
  ...
  for (let i = 0; i < 2; i++) {
    ...
    const observed = await call("agent_task", { action: "observe", agentId: record.id, waitMs: 0 });
  ...
  for (;;) {
    const observed = await call("agent_task", { action: "observe", agentId: record.id, waitMs: 0, includeResponse: true, responseOffset });
  ...
  const largeSuccess = await call("agent_task", { action: "observe", agentId: record.id, waitMs: 0, includeResponse: true });
  assert.equal(largeSuccess.status, "completed"); assert(largeSuccess.responsePage.nextOffset > 0);
  const foreign = await client.callTool({ name: "work_task", arguments: { action: "get", workspaceId: "ws_foreign", workRunId: run.id } }); assert(foreign.isError);
  const foreignObserve = await client.callTool({ name: "agent_task", arguments: { action: "observe", workspaceId: "ws_abcdef", workRunId: "run_foreign", agentId: record.id, waitMs: 0 } }); assert(foreignObserve.isError);
  const expired = await client.callTool({ name: "write_stdin", arguments: { workspaceId: "ws_abcdef", sessionId: terminal.sessionId } }); assert(expired.isError);
  ...
  assert(completed.some((e) => e.operationId === patch.operationId));
