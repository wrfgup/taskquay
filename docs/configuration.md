...
`agent_task` provides start/continue/observe/list/claims/usage/cancelQueued plus exact-active-turn `steer` and `interrupt`. Control requires workspace, work run, agent, a new request key and the observed provider turn id. It is sent only through the live owning runtime; stale identity and unconfirmed transport results fail closed without replay. Observe locally waits up to 25 seconds with revision deduplication; full response expansion is explicit. Usage retains provider snapshots and unknowns, not a fabricated invoice. Queued tasks can be cancelled before invocation. Desktop takeover/return remains owner-only in the local console rather than a remote MCP action.
...
Daemon protocol version **8** prevents an older daemon from ignoring queued states,
context/request fields, work-run ownership, persisted turns, event-driven waits,
provider launch environments, provider-config revisions, or active-turn control identity. Interrupted tasks are
